...
graph3 <- data %>% 
  filter(year == 2020) %>%
  ggplot() +
  aes(x = outstanding_deposits_with_commercial_banks_percent_of_gdp , y = economy) +
  geom_col(aes(fill = outstanding_deposits_with_commercial_banks_percent_of_gdp), width = 0.7)
graph3
```
\newpage

## Graph 4:
NOTE TO SYLVIA: the most up-to-date graph is on Pierina's laptop
```{r, results = FALSE}
graph4 <- data %>% 
   ggplot() +
  aes(x = number_of_commercial_bank_branches_per_100_000_adults, y = outstanding_deposits_with_commercial_banks_percent_of_gdp) +
  geom_point() +
  theme_minimal() +
...
animate(graph4, duration = 17, fps = 17, width = 200, height = 200, renderer = gifski_renderer())
graph4
```
